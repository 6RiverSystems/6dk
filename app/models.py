...
class User(UserMixin, PaginatedAPIMixin, db.Model):
    ...
    active_profile = db.Column(
        db.String(128), db.ForeignKey('profile.token_id'))
    ...
    def new_messages(self):
        ...
        token_ids = [profile['token_id']
                     for profile in self.load_user_profiles()]
    ...
    def get_forward_profile_token(self, destination, profile_token,
                                  expires_in=600):
        return jwt.encode(
            {
                'source': self.id,
    ...
    def receive_forward_profile_token(self, jwt_token):
        try:
            ...
            if token_data['destination'] == self.id \
                    and User.query.get(token_data['source']).owns_token(
                        token_data['profile']) \
...
class Profile(PaginatedAPIMixin, db.Model):
    token_id = db.Column(db.String(128), index=True,
...
class Message(PaginatedAPIMixin, db.Model):
    ...
    def get_replays(message_id):
        transmissions = MessageTransmission.query.filter_by(
